...
## Clean minimal viewing phase UI — white text, no background panels.

@onready var bottom_center: VBoxContainer = $BottomCenter
@onready var ready_button: Button = $BottomCenter/ReadyButton
@onready var status_label: Label = $BottomCenter/StatusLabel
...
func update_waiting_count(ready_count: int, total_count: int) -> void:
	...
	if ready_count < total_count:
		status_label.text = "%d / %d ready" % [ready_count, total_count]
	else:
		status_label.text = "All ready — starting game..."
